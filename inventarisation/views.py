...
class InventarisationCreate(PermissionRequiredMixin, CreateView):
    ...
    success_url = reverse_lazy('inventarisations.inventarisation.list')
...
def inventarisation_form(request, inventarisation_id, page_id):
    ...
    if len(item_pages) == 0:
        return HttpResponseRedirect(reverse('inventarisations.inventarisation.finish', args=[inventarisation_id]))
...
def get_inventarisation_next(request, inventarisation_id, page_id):
    ...
    if page_id > -2:
        return HttpResponseRedirect(reverse('inventarisations.inventarisation.by_number', args=(inventarisation_id, page_id)))
    page_id = get_cur_block(inventarisation, -1)
    if page_id == -2:
        return HttpResponseRedirect(reverse('inventarisations.inventarisation.finish', args=[inventarisation_id]))
    else:
        return HttpResponseRedirect(reverse('inventarisations.inventarisation.early', args=[inventarisation_id]))
