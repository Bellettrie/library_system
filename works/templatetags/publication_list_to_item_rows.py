...
def get_item_rows_for_publications(publications: List[Work])-> List[RowData]:
    ...
    for publication in publications:
        if hasattr(publication, 'itemid') and publication.itemid and hasattr(publication, 'book_code') and hasattr(publication, 'book_code_extension'):
            item = Item(publication=publication, book_code=publication.book_code, book_code_extension=publication.book_code_extension, id=publication.itemid)
